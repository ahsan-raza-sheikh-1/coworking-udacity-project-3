...
# Temporary source storage for cloud builds.
resource "aws_s3_bucket" "codebuild_source" {
  bucket       = "movie-picture-pipeline-build-${data.aws_caller_identity.current.account_id}"
  force_destroy = true
}
...
resource "aws_iam_role_policy" "codebuild_image_push" {
  name = "codebuild-image-push"
  role = aws_iam_role.codebuild.name
