...
The workflows in `.github/workflows` use GitHub Actions secrets and variables rather than storing AWS or application values in the repository. Add these repository secrets before running either deployment workflow:
...
You can retrieve the backend address after deployment with:

```bash
kubectl get service backend --namespace default
```

The CI workflows run on pull requests targeting `main` and can also be started manually. The CD workflows run on pushes to `main` and can also be started manually. Each deployment tags its ECR image with the triggering commit SHA, applies the Kubernetes manifests, updates the deployment image, and waits for the rollout to complete.

The local `.env` file is ignored by the root `.gitignore` and must never be committed. Any AWS values that have been exposed in a local `.env` file should be revoked and replaced before using them in GitHub Secrets.
